...
class Quota:
    quota_id: str
    metric: str  # requests, input_tokens, output_tokens, total_tokens
    limit: float
    used: float = 0
    reserved: float = 0
...
class QuotaLedger:
    ...
    def quota_shadow_cost(self, route_id: str, replacement_cost: float, criticality: str) -> float:
        ...
        # Low priority gets greater scarcity penalty; critical work can consume reserved premium free capacity.
        factor = {
            "disposable": 1.0,
            "routine": 0.9,
            "important": 0.5,
