...
import random
from .types import TaskSpec, Route, CandidateAssessment, ExecutionPlan
from .policy import hard_exclusions
...
class HotSwapRouter:
    ...
    def _dominates(a: CandidateAssessment, b: CandidateAssessment) -> bool:
        """Conservative Pareto relation over success, expected cost, reliability and latency."""
    ...
    def plan(self, task: TaskSpec, routes: list[Route], rng: random.Random | None = None) -> ExecutionPlan:
        ...
        # Use strongest available route's raw call cost as a simple escalation proxy.
        ...
        # Quality floor uses conservative lower bound for release/production; mean otherwise.
        qualified = []
        for a in assessed:
        ...
        # Stable tie-breaker after ECPS: better lower bound, lower latency.
        ...
        # Fallbacks can include the rest of qualified frontier, not only same free tier.
